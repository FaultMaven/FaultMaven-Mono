...
To design and implement a high-performance, scalable, and consistent frontend for FaultMaven — an AI-powered troubleshooting assistant. This includes:
...
├── docs/               # Architecture/Design Docs (like this one)
├── packages/           # Shared internal packages (monorepo)
│   ├── ui/             # Shared UI components (@faultmaven/ui source)
│   └── types/          # Shared TypeScript types (optional @faultmaven/types)
├── frontend/           # Web App (Next.js)
├── scripts/            # Utility scripts
├── tests/              # Backend tests
├── docker-compose.yml  # For local dev environment
├── Dockerfile          # Backend Dockerfile
├── package.json        # Root package for monorepo tooling
...
│   └── ...             # Other Plasmo components
├── public/             # Static assets for extension
├── package.json        # Extension dependencies (incl. @faultmaven/ui)
├── tsconfig.json
└── README.md
```
...
## 🔄 Code Sharing & Integration Workflow
...
- ✅ Lightweight, auditable browser extension
- ✅ Reusable, versioned components across frontend/extension
- ✅ Independent tech evolution

---

## 🌐 Web Frontend (`faultmaven/frontend/`)
...
## 🧩 Browser Extension (`faultmaven-extension/`)
...
├── public/                       # Static assets (icons, etc.) copied to extension root
│   └── icon/                     # Extension icons
├── src/                          # Primary source code
│   ├── assets/                   # CSS, images used by TSX/JS
│   │   └── styles/
│   │       └── globals.css       # Global styles with Tailwind CSS
│   ├── entrypoints/              # WXT entry scripts
│   │   ├── background.ts         # Background service worker (session, side panel trigger)
│   │   ├── page-content.content.ts # Auto-injected content script for page data
│   │   └── sidepanel_manual/     # Directory for manually structured side panel
│   │       ├── index.html        # HTML shell for the side panel
│   │       └── main.tsx          # React mounter script for SidePanelApp
│   ├── lib/                      # Utility functions shared within the extension
│   │   └── utils/
│   │       ├── config.ts
│   │       └── messaging.ts
│   ├── shared/                   # Shared React components/logic within the extension
│   │   └── ui/
│   │       └── SidePanelApp.tsx  # Main React UI component for the side panel
├── package.json                  # Extension dependencies (incl. @faultmaven/ui)
├── pnpm-lock.yaml                # Or other lock file
...
- **Functionality:**
  ...
  - **Page Interaction:**
    - An auto-injected content script (`src/entrypoints/page-content.content.ts`) matching relevant URLs (e.g., initially `https://www.example.com/*` for development—with plans to expand to `<all_urls>` for production after resolving dev HMR stability) listens for messages from the side panel.
